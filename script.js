...
// No dark mode toggle per feedback
...
function generateQuote() {
    ...
    document.getElementById('daily-quote').innerHTML = `<span class="not-italic font-bold text-[#00A89D]">"</span>${randomQuote}<span class="not-italic font-bold text-[#00A89D]">"</span>`;
}
generateQuote();
...
function calculatePayment() {
    ...
    const pmi = (P * parseFloat(document.getElementById('pmi').value) / 100) / 12;
    const payment = P * r * Math.pow(1 + r, n) / (Math.pow(1 + r, n) - 1);
    const total = payment + taxes + insurance + pmi;
    document.getElementById('calc-output').innerHTML = `<div class="grid grid-cols-1 gap-4 text-left">
        <p><strong>Principal & Interest:</strong> $${payment.toFixed(2)}</p>
        <p><strong>Taxes:</strong> $${taxes.toFixed(2)}</p>
        <p><strong>Insurance:</strong> $${insurance.toFixed(2)}</p>
        <p><strong>PMI:</strong> $${pmi.toFixed(2)}</p>
        <p class="font-bold text-[#F15A29]">Total Monthly: $${total.toFixed(2)}</p>
    </div>`;
}