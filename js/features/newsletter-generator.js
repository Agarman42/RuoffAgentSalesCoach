...
function buildPersonalPhotoInsert(photoUrl, widthPx) {
    ...
    // (width:100% tables with a smaller img often look left-biased when the parent is left-aligned).
...
/** Force every personal-photo variant onto ONE canonical centered block. */
function ensurePersonalPhotoCentered(htmlString, photoUrl) {
    ...
    const block = buildPersonalPhotoInsert(src);
    if (!block) return out;
...
function cancelPersonalMediaSliderRaf() {
    if (_nlMediaSliderRaf) {
        cancelAnimationFrame(_nlMediaSliderRaf);
        _nlMediaSliderRaf = 0;
    }
}
...
/** Form-side thumbs only — never touches #nl-preview / srcdoc / contentDocument. */
function applyPersonalMediaFormThumbCss() {
    const photoPx = getPersonalPhotoWidthPx();
    const videoPx = getPersonalVideoWidthPx();
    const photoLabel = document.getElementById('nl-personal-photo-size-label');
    const videoLabel = document.getElementById('nl-personal-video-size-label');
    if (photoLabel) photoLabel.textContent = `Photo size · ${photoPx}px`;
    if (videoLabel) videoLabel.textContent = `Video size · ${videoPx}px`;

    const photoImg = document.getElementById('nl-personal-photo-preview-img');
    if (photoImg) {
        photoImg.style.width = photoPx + 'px';
        photoImg.style.maxWidth = '100%';
        photoImg.style.height = 'auto';
    }
    ...
    const videoThumb = document.getElementById('nl-personal-video-preview-thumb');
    if (videoLink) {
        videoLink.style.width = videoPx + 'px';
        videoLink.style.maxWidth = '100%';
        videoLink.style.display = 'block';
        videoLink.style.boxSizing = 'border-box';
        const iframe = videoLink.querySelector('iframe');
        if (iframe) {
            const base = parseFloat(iframe.dataset.nlDragBaseW) || iframe.offsetWidth || videoPx;
            if (!iframe.dataset.nlDragBaseW) iframe.dataset.nlDragBaseW = String(base || 1);
            const scale = base ? videoPx / base : 1;
            iframe.style.transformOrigin = 'top center';
            iframe.style.transform = scale === 1 ? '' : `scale(${scale})`;
        } else if (videoThumb) {
            videoThumb.style.width = videoPx + 'px';
            videoThumb.style.maxWidth = '100%';
            videoThumb.style.height = 'auto';
        }
    }
...
function applyPersonalMediaLiveSize() {
    applyPersonalMediaFormThumbCss();
}
...
function commitPersonalMediaSize() {
    ...
    try {
        ...
        const videoIframe = document.querySelector('#nl-personal-video-preview-link iframe');
        if (videoIframe) {
            videoIframe.style.transform = '';
            delete videoIframe.dataset.nlDragBaseW;
        }
...
function wirePersonalMediaSizeSliders() {
    ['nl-personal-photo-size', 'nl-personal-video-size'].forEach((id) => {
        ...
        const onInput = () => {
            ...
            if (_nlMediaSliderRaf) return;
            _nlMediaSliderRaf = requestAnimationFrame(() => {
                _nlMediaSliderRaf = 0;
                applyPersonalMediaFormThumbCss();
            });
        };
        ...
        el.addEventListener('pointerdown', () => {
            _nlMediaSliderDrag = true;
            const iframe = document.querySelector('#nl-personal-video-preview-link iframe');
            if (iframe && !iframe.dataset.nlDragBaseW) {
                iframe.dataset.nlDragBaseW = String(iframe.offsetWidth || getPersonalVideoWidthPx() || 1);
            }
        });
        el.addEventListener('pointerup', onRelease);
        el.addEventListener('pointercancel', onRelease);
        el.addEventListener('blur', () => {
            if (!_nlMediaSliderDrag && !_nlMediaSliderRaf) return;
            onRelease();
        });
    });
}
...
function getNewsletterHtmlForFeedbackEdit() {
    ...
    if (!base) return base;
    return compactNewsletterHtmlForModel(stripFooterModulesForReEdit(base), 14000);
}
...
<style id="nl-preview-fluid">
  ...
  */
  ...
  /* Photo insert: content-sized + auto margins (never left-float in a left-aligned note card) */
...
function patchPersonalMediaSizesInNewsletter() {
    ...
    if (photoEnabled && photoUrl) {
        const next = ensurePersonalPhotoCentered(html, photoUrl);
...
function resolvePersonalVideoThumb(href) {
    ...
    if (yt) return { kind: 'youtube', url: `https://img.youtube.com/vi/${yt}/mqdefault.jpg`, label: 'YouTube' };
    if (isInstagramVideoUrl(href)) return { kind: 'instagram', url: null, label: 'Instagram' };
...
function buildPersonalVideoThumbCell(href, videoWidthPx, thumbMeta) {
    ...
    const sub = brand === 'Instagram' ? 'Tap to open Reel / post on Instagram' : 'Tap to open video';
...
function stripCustomNewsletterSection(html) {
    let out = String(html || '');
    out = out.replace(
        /<tr>\s*<td[^>]*>\s*<table[^>]*data-nl-custom-section=["']1["'][^>]*>[\s\S]*?<\/table>\s*<\/td>\s*<\/tr>\s*(?:<tr>\s*<td[^>]*height=["']?(?:16|20)["']?[^>]*>\s*<\/td>\s*<\/tr>\s*)?/gi,
        ''
    );
    out = out.replace(
        /<table[^>]*data-nl-custom-section=["']1["'][^>]*>[\s\S]*?<\/table>\s*(?:<tr>\s*<td[^>]*height=["']?(?:16|20)["']?[^>]*>\s*<\/td>\s*<\/tr>\s*)?/gi,
        ''
    );
    return out;
}

function findInsertBeforePersonalIndex(html) {
    const src = String(html || '');
    ...
    while (searchFrom > 0) {
        ...
        const isOuterCard = /<tr[\s>][\s\S]*?<table\b/i.test(segment)
            && /<h2[^>]*>\s*A\s*Note\s*From\b/i.test(segment);
        if (isOuterCard) {
            let insertAt = trPos;
            const beforeCard = src.substring(0, insertAt);
            const spacerRe = /<tr>\s*<td[^>]*height=["']?\d+["']?[^>]*>\s*<\/td>\s*<\/tr>\s*$/i;
...
function injectCustomSectionBeforePersonal(html, innerTableHtml) {
    ...
    if (out.includes('<!-- CUSTOM SECTION PLACEHOLDER -->')) {
        return out.replace('<!-- CUSTOM SECTION PLACEHOLDER -->', section);
    }

    const personalIdx = findInsertBeforePersonalIndex(out);
    if (personalIdx >= 0) {
        return out.slice(0, personalIdx) + section + out.slice(personalIdx);
    }

    const personalComment = /<!--\s*Personal Note Section\s*-->/i;
    if (personalComment.test(out)) {
        return out.replace(personalComment, section + '\n<!-- Personal Note Section -->');
    }
    ...
    const sigTable = /(<table\b[^>]*data-nl-signature-block=["']1["'][^>]*>)/i;
    if (sigTable.test(out)) {
        return out.replace(sigTable, section + '\n$1');
    }
    const disclaimerTable = /(<table\b[^>]*data-nl-disclaimer-block=["']1["'][^>]*>)/i;
    if (disclaimerTable.test(out)) {
        return out.replace(disclaimerTable, section + '\n$1');
    }
    const referral = /<tr>\s*<td[^>]*>[\s\S]*?Know Someone Thinking About Buying or Selling\?[\s\S]*?<\/tr>/i;
    if (referral.test(out)) {
        return out.replace(referral, section + '$&');
    }
    const module = `<table width="600" align="center" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:600px;margin:0 auto;border-collapse:collapse;">${section}</table>`;
    if (/<\/body>/i.test(out)) return out.replace(/<\/body>/i, module + '</body>');
    return out + module;
}

function applyNewsletterCustomSection(html, custom) {
    ...
    out = injectCustomSectionBeforePersonal(out, buildCustomNewsletterSectionTable(title, body));
...
async function prepareCustomSectionBody(title, body, options) {
    ...
    try {
        const prompt = asBrief
            ? [
                'You write ONE short real-estate newsletter section for a local realtor / listing agent.',
                ...
                'Stay on-brief. No extra headings, no guarantees, no invented statistics.',
                'No mortgage rates, APR, lender, refinance, or loan-officer language.',
            ...
            : [
                'Lightly polish this realtor newsletter section for grammar and clarity only.',
                'Keep the author\'s meaning and specifics. Do not add claims, guarantees, new facts, headings, or disclaimers.',
                'No mortgage rates, APR, or lender-only language. Do not rename the section.',
...
function updatePersonalMediaPreviews() {
    if (isPersonalMediaSliderDragging()) {
        applyPersonalMediaFormThumbCss();
        return;
    }
    updatePersonalPhotoSizeUI();
    ...
    if (videoWrap && videoThumb && videoLink && videoStatus) {
        if (!videoEnabled || !videoUrl) {
            ...
            if (thumbMeta.kind === 'youtube' && thumbMeta.url) {
                ...
                videoThumb.removeAttribute('src');
                videoStatus.innerHTML = '<span class="text-amber-700">Link will open in email; YouTube gets a real thumb, Instagram gets a branded card</span>';
            }
            applyPersonalVideoPreviewSizing();
        }
    }
...
function updateNewsletterPreflightSummary() {
    ...
    if (customState.enabled) {
        if (customState.ready) {
            chips.push({ text: `Custom · ${customState.title}`, style: 'included', removeId: 'nl-custom-section' });
        } else {
        ...
        if (html && html.trim() !== '') {
            ...
            if (!feedback) {
                ...
                if (includePhoto && personalPhotoUrl) {
                    ...
                    html = stripPersonalPhotoVariants(html, '');
                    html = html.replace(/\[PERSONAL PHOTO PLACEHOLDER\]/gi, '');
                }
...
html = applyUncheckedNewsletterSectionFilters(html, postSelections);
            ...
            if (!customInjectPayload && typeof getNewsletterCustomSection === 'function') {
                const late = getNewsletterCustomSection();
...
function copyForOutlook() {
  ...
  function normalizePersonalPhotoBlocks(htmlString) {
      ...
      // margin:16px 0 0 without auto, which left-biased photos in wide preview shells.
  ...
  function normalizeSectionBorders(doc) {
      ...
      tables.forEach(table => {
          ...
          // Personal note card may contain a centered photo — don't force nested media left
          const hasPersonalPhoto =
              !!table.querySelector('table[data-nl-personal-photo="1"], img[alt="Personal photo"]');
          ...
          if (firstTd) {
              ...
              } else if (!hasPersonalPhoto) {
                  firstTd.style.textAlign = 'left';
              }
          }
          if (hasPersonalPhoto) {
              table.querySelectorAll('table[data-nl-personal-photo="1"]').forEach((pt) => {
                  pt.style.marginLeft = 'auto';
                  pt.style.marginRight = 'auto';
                  pt.style.width = 'auto';
                  pt.style.maxWidth = '100%';
                  pt.style.float = 'none';
              });
              table.querySelectorAll('img[alt="Personal photo"]').forEach((img) => {
          ...
          if (!isPersonalVideo) {
              table.querySelectorAll('h2, h3, h4, p, li, ul, ol, div').forEach((el) => {
                  ...
                  if (el.closest('table[data-nl-personal-photo="1"]')) return;
